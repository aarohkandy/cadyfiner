...
**Status: both models trained and evaluated end-to-end. Every number in this document is real,
measured on this project's own harness — none are projected or hypothetical. Read Section 5
before trusting either model: the honest headline is that neither trained model has yet
demonstrated a clear win over the general-purpose backend it was meant to specialize past, for
two different reasons (Model 2: a genuinely hard task at either scale; Model 1: the evaluation
itself is bottlenecked by the CAD-code generator's own reliability, not by Stage 2). This is not
a failure to hide — it's exactly the kind of result this project's own methodology exists to
...
**Model 1 — the real end-to-end comparison (`scripts/run_harness_trained.py`).** All 13
held-out seeds (`heldout_same_family` + `heldout_family`, the same split the general-purpose
model was evaluated against elsewhere in this project), CAD code generated by `gemma4:e4b` for
both arms, Stage 2 filled by the trained model instead of the general-purpose one.

| | result |
|---|---|
| pairs run | 13 |
| excluded (Stage 2 produced no usable refined prompt, fell back to Stage-1-only) | 1 |
| valid pairs | 12 |
| refined wins | 1 (`pen_holder_high`: raw failed, refined passed) |
| raw wins | 0 |
| ties (both arms failed Leg 1) | 11 |
| win rate among decisive pairs | 100% (1/1) |
| 95% CI | [21%, 100%] |
| sign-test p | 1.000 |
| **verdict** | **underpowered — not a negative result, not a positive one** |

**Read this honestly.** A 100% win rate sounds good and is almost meaningless here: it's one
decisive pair out of twelve. The real story is in the *ties* — 11 of 12 valid pairs failed Leg 1
**regardless of which prompt they got**. That is not a Stage-2 quality problem, trained or
general-purpose; it means `gemma4:e4b`'s own CAD-code generation is failing outright on most of
these held-out seeds, and no amount of prompt refinement changes an output the generator can't
produce correctly in the first place. This directly echoes a finding already in the main
README (evidence #4, the mechanical pilot on `dolphincoder:7b`): **a weak local generator is a
ceiling that masks any refinement effect, whichever backend fills Stage 2.** The one seed where
refinement *did* flip a failure into a pass (`pen_holder_high`, a decorative/simple-geometry
family) is a plausible, encouraging data point — but n=1 cannot support a general claim, and the
honest report is "not enough signal to say," not "it works" or "it doesn't."

What this does and doesn't tell us about Model 1 specifically: it tells us nothing yet about
whether the *trained* Stage-2 model is better or worse than the general-purpose one — with the
generator failing on 11/12 seeds regardless, this run cannot distinguish the two. A meaningful
comparison needs either a stronger/more reliable CAD-code generator on the held-out seeds, or
many more reps per seed to accumulate enough decisive pairs through the noise. Both are real
...
- Both models have now been evaluated end-to-end (Section 5), and neither has demonstrated a
  clear win: Model 2 underperforms a trivial constant baseline (as does the general-purpose
  model it was compared against — this looks like a task-framing problem, not a Model-2-specific
  one); Model 1's comparison is statistically underpowered because the downstream CAD-code
  generator fails on 11 of 12 held-out seeds regardless of which prompt it receives, leaving only
  one decisive data point. **Treat both as "built, plumbed in, and honestly inconclusive-to-negative
  so far," not "proven better."** The most useful next step for Model 1 specifically is re-running
  this same comparison against a more reliable CAD-code generator (or with more reps per seed) —
  the bottleneck identified here is the generator, not the refiner.
